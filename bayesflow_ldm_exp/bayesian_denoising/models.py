...
class DenoisingApproximator(bf.ContinuousApproximator):
...
SPECS = {
    "spatial_direct": ModelSpec(
        ...
        family="spatial",
        target_key="image",
        condition_key="observed",
        batch_size=C.SPATIAL_BATCH_SIZE,
        sample_batch_size=C.SPATIAL_SAMPLE_BATCH_SIZE,
    ),
    "spatial_ldm_14x14x2": ModelSpec(
        key="spatial_ldm_14x14x2",
        name=C.DISPLAY_NAMES["spatial_ldm_14x14x2"],
        family="spatial",
        target_key="image",
        condition_key="observed",
        batch_size=C.SPATIAL_BATCH_SIZE,
    ...
    "spatial_ldm_7x7x4": ModelSpec(
        key="spatial_ldm_7x7x4",
        name=C.DISPLAY_NAMES["spatial_ldm_7x7x4"],
        family="spatial",
        target_key="image",
        condition_key="observed",
        batch_size=C.SPATIAL_BATCH_SIZE,
...
def build_inference_network(key):
    ...
    if key == "spatial_ldm_14x14x2":
        return _spatial_latent_diffusion(latent_shape=(14, 14, 2), downsample_steps=1)
    if key == "spatial_ldm_7x7x4":
        return _spatial_latent_diffusion(latent_shape=(7, 7, 4), downsample_steps=2)
